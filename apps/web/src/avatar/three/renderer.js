// GENERATED by apps/web/scripts/extract-renderer.mjs from
...
export function createAvatarRenderer({ container, modelUrl, backdropUrl = null, debug = false, onMessage = () => {} } = {}) {
  ...
  function frameCamera(modelScene) {
    ...
    _dbg('Camera framed: h=' + height.toFixed(2) + ' dist=' + distance.toFixed(2));
  }
  ...
  api.setAvatarState = setAvatarState;
  ...
  function patchExprMap() {
    ...
    if (!('viseme_aa' in dict) && 'aa' in dict && 'ih' in dict) {
      ...
      _dbg('Visemes: AvatarSDK full set (vowels + consonant articulators)');
    }
  ...
  function loadModel() {
    _dbg('Renderer created, loading GLB model...');
  ...
  async function loadBackdrop() {
    ...
    try {
      ...
      _dbg('Backdrop: Blob ready (' + Math.round(blob.size / 1024 / 1024) + ' MB), loading GLB...');
    } catch (e) {
      _dbg('Backdrop: Blob conversion failed — ' + (e && e.message ? e.message : String(e)));
      return;
    }
    ...
    backdropLoader.load(
      ...
      (gltf) => {
        ...
        _dbg('Backdrop: GLB parsed, building scene...');
        ...
        _dbg('Backdrop: added to scene');
      },
      (xhr) => {
        if (xhr.total) {
          const pct = Math.round((xhr.loaded / xhr.total) * 100);
          if (pct === 25 || pct === 50 || pct === 75 || pct === 100) {
            _dbg('Backdrop loading: ' + pct + '%');
          }
        }
      },
      (err) => {
    ...
    try {
      ...
      src.onended = function() {
        ...
        if (s.endRequested && s.pending === 0 && streamSession === s) {
          ...
          _stopCurrent();
          if (_bridge) {
            _bridge.postMessage(JSON.stringify({ type: 'audioEnd', underruns: underruns }));
          }
        }
      };
