...
export default function Home() {
  ...
  const featured = S.FEATURED.map((f) => {
    ...
    return {
      title: a.title,
      mins: f.mins + ' min',
      catName: S.getCat(a.cat).name,
      chipBg: y.bg,
      chipFg: y.fg,
      open: go('#/app/library/' + a.id),
    };
  });
  ...
  return (
    <section
      style={{
        height: isMobile ? 'calc(100dvh - 96px)' : '100dvh',
        boxSizing: 'border-box',
        display: 'flex',
        flexDirection: 'column',
        gap: isMobile ? '10px' : '14px',
        padding: isMobile ? '12px 0 4px' : '20px 0 18px',
        overflow: 'hidden',
      }}
    >
      <div style={{ flexShrink: 0 }}>
        <h1
          style={{ fontSize: 'clamp(1.25rem, 2.4vw, 1.8rem)', margin: 0, letterSpacing: '-.01em' }}
        >
          {greeting}
        </h1>
        {!isMobile && (
          <p
            style={{
              margin: '4px 0 0',
              color: 'var(--text2)',
              fontSize: 'clamp(.9rem, 1.3vw, 1.02rem)',
              textWrap: 'pretty',
            }}
          >
      ...
      >
        ...
        <div
          style={{
            flex: isMobile ? '0 0 32%' : '0 0 clamp(250px, 26vw, 380px)',
            minHeight: 0,
            borderRadius: '22px',
            border: 'var(--bw) solid var(--border)',
            background:
              'radial-gradient(circle at 50% 28%, var(--tint) 0%, transparent 75%), linear-gradient(160deg, var(--tint), var(--surface))',
            boxShadow: 'var(--shadow)',
            overflow: 'hidden',
          }}
        >
        ...
        >
          ...
          >
            {[
              ['Chat', 'Ask questions', '#/app/chat'],
              ['Voice', `Talk with ${who}`, '#/app/voice'],
              ['Library', '49 curated articles', '#/app/library'],
              ['Settings', 'Make it yours', '#/app/settings'],
            ].map(([t, sub, r]) => (
              <button
                key={t}
                onClick={go(r)}
              ...
              >
                <div
                  style={{
                    fontWeight: 700,
                    fontSize: 'clamp(.92rem, 1.3vw, 1.02rem)',
                    marginBottom: '2px',
                  }}
                >
                  {t}
              ...
              {featured.map((f) => (
                <button
                  ...
                  style={{
                    ...card,
                    flex: '0 0 auto',
                    width: isMobile ? '180px' : 'clamp(190px, 21vw, 250px)',
                    textAlign: 'left',
                    padding: '12px 14px',
                    cursor: 'pointer',
                    color: 'var(--text)',
