...
const ToggleRow = ({ label, sub, on, onToggle, last }) => (
  <div
    style={{
      ...
      minHeight: '64px',
      padding: '12px 0',
      borderBottom: last ? 'none' : 'var(--bw) solid var(--border)',
    }}
  >
    <span style={{ flex: '1' }}>
      <span style={{ display: 'block', fontWeight: '600' }}>{label}</span>
      <span style={{ display: 'block', color: 'var(--text2)', fontSize: '.92rem' }}>{sub}</span>
    </span>
...
export default function Settings() {
  ...
  const avRows = [
    ...
    [
      ...
      `${who} notices when you finish speaking — no need to tap stop`,
    ],
    [
      'fasterVoice',
      'Faster Voice Responses',
      `${who} starts speaking sooner — turn off if audio sounds choppy`,
    ],
  ];
    ...
    askConfirm(
      {
        ...
        message: 'This removes all past conversations from this browser. It cannot be undone.',
  ...
  return (
    <section style={{ padding: '28px 0 48px', maxWidth: '760px' }}>
      ...
      <Card>
        ...
        >
          <span style={{ flex: '1', minWidth: '180px' }}>
            <span style={{ display: 'block', fontWeight: '600' }}>Text Size</span>
          ...
          >
            {[
              ...
              <button
                key={v}
                onClick={() => setSetting('textScale', v)}
                aria-label={`Text size ${v === 0.85 ? 'small' : v === 1 ? 'medium' : 'large'}`}
                style={{
                  minWidth: '52px',
                  minHeight: '44px',
                  borderRadius: '9px',
        ...
        <ToggleRow
          ...
          onToggle={() => setSetting('highContrast', !settings.highContrast)}
        />
        <ToggleRow
          last
          label="Dark Mode"
          ...
          on={effDark}
          onToggle={() => setSetting('darkMode', !effDark)}
        />
      </Card>
      ...
      <Card>
        ...
        >
          <span style={{ flex: '1', minWidth: '180px' }}>
            <span style={{ display: 'block', fontWeight: '600' }}>Avatar</span>
            <span style={{ display: 'block', color: 'var(--text2)', fontSize: '.92rem' }}>
              Who you talk with
            </span>
          </span>
            ...
            {Object.values(AVATAR_PROFILES).map((p) => {
              ...
              return (
                <button
                  key={p.id}
                  onClick={() => {
                    if (!unityLocked) setSetting('avatarId', p.id);
                  }}
                  disabled={unityLocked}
                  title={
                    unityLocked
                      ? 'Unity WebGL build not installed — see public/unity/README.md'
                      : p.description
                  }
                  style={{
                    minHeight: '44px',
                    ...
                    borderRadius: '9px',
                    border: 'none',
                    cursor: unityLocked ? 'not-allowed' : 'pointer',
                    fontWeight: '600',
                    fontSize: '.9rem',
                    opacity: unityLocked ? 0.45 : 1,
                    ...segStyle(on),
                  }}
                >
                  {p.name}
        ...
        {avRows.map(([k, label, sub]) => (
          <ToggleRow
            ...
            sub={sub}
            on={!!settings[k]}
            onToggle={() => setSetting(k, !settings[k])}
          />
        ))}
        <div
          style={{
        ...
        >
          <span style={{ flex: '1', minWidth: '180px' }}>
            <span style={{ display: 'block', fontWeight: '600' }}>Voice Speed</span>
            <span style={{ display: 'block', color: 'var(--text2)', fontSize: '.92rem' }}>
              How fast {who} speaks
            </span>
          </span>
          ...
          >
            {[
              ...
              <button
                key={v}
                onClick={() => setSetting('voiceSpeed', v)}
                style={{
                  minHeight: '44px',
                  padding: '0 14px',
                  borderRadius: '9px',
                  ...
                  fontWeight: '600',
                  fontSize: '.9rem',
                  ...segStyle(settings.voiceSpeed === v),
                }}
              >
                {label}
              </button>
      ...
      <Card>
        ...
        >
          <span style={{ flex: '1' }}>
            <span style={{ display: 'block', fontWeight: '600' }}>Privacy Policy</span>
            <span style={{ display: 'block', color: 'var(--text2)', fontSize: '.92rem' }}>
        ...
        <button
          onClick={() =>
            showToast('Conversations are stored only on this device — nothing is sent to a server.')
          }
          style={{
            display: 'flex',
            alignItems: 'center',
        ...
        >
          <span style={{ flex: '1' }}>
            <span style={{ display: 'block', fontWeight: '600' }}>Data Security</span>
            <span style={{ display: 'block', color: 'var(--text2)', fontSize: '.92rem' }}>
        ...
        >
          <span style={{ flex: '1' }}>
            <span style={{ display: 'block', fontWeight: '600' }}>Medical Disclaimer</span>
            <span style={{ display: 'block', color: 'var(--text2)', fontSize: '.92rem' }}>
              What Aria can and can't help with
            </span>
          </span>
          <span style={{ color: 'var(--text2)' }}>›</span>
        </a>
        ...
        >
          <span style={{ flex: '1' }}>
            <span style={{ display: 'block', fontWeight: '600' }}>Clear Conversation History</span>
            <span style={{ display: 'block', color: 'var(--text2)', fontSize: '.92rem' }}>
              Remove past conversations from this device
            </span>
          </span>
          <span style={{ color: 'var(--text2)' }}>›</span>
        </button>
      ...
      <Card>
        ...
        >
          <span style={{ flex: '1' }}>
            <span style={{ display: 'block', fontWeight: '600' }}>API Keys</span>
            <span style={{ display: 'block', color: 'var(--text2)', fontSize: '.92rem' }}>
              For developers — connect live services
            </span>
          </span>
          <span style={{ color: 'var(--text2)' }}>{advOpen ? '▾' : '▸'}</span>
        </button>
        {advOpen && (
          <div
            style={{ display: 'flex', flexDirection: 'column', gap: '14px', padding: '4px 0 18px' }}
          >
            {keyRows.map(([k, label]) => (
            ...
            <p style={{ margin: '0', color: 'var(--text2)', fontSize: '.85rem' }}>
              Keys are stored only in this browser — never sent anywhere else. With an OpenAI key
              saved, Aria answers with the live knowledge base; without one she uses built-in
              example answers.
            </p>
      ...
      <Card mb={0}>
        ...
        >
          <span style={{ flex: '1' }}>
            <span style={{ display: 'block', fontWeight: '600' }}>Version</span>
            <span style={{ display: 'block', color: 'var(--text2)', fontSize: '.92rem' }}>
              v1.0 · Made in Aotearoa NZ
            </span>
          </span>
        </div>
        <div
        ...
        >
          <span style={{ flex: '1' }}>
            <span style={{ display: 'block', fontWeight: '600' }}>Project</span>
            <span style={{ display: 'block', color: 'var(--text2)', fontSize: '.92rem' }}>
              University of Auckland Part IV Software Engineering project
            </span>
          </span>
        </div>
        <button
        ...
        >
          <span style={{ flex: '1' }}>
            <span style={{ display: 'block', fontWeight: '600', color: 'var(--primary-d)' }}>
              Redo the setup questions
            </span>
            <span style={{ display: 'block', color: 'var(--text2)', fontSize: '.92rem' }}>
              Run the guided onboarding again
            </span>
          </span>
          <span style={{ color: 'var(--text2)' }}>›</span>
        </button>
