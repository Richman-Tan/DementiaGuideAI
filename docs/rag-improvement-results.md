...
| Recall@{1,3}, precision@5, MRR, nDCG@5 | *to be backfilled from baseline audit.json once metrics module lands (Stage 6)* | 32 | Deterministic |
| Safety pass-rate (S-set), injection pass-rate (I-set), NZ set | *no baseline exists — sets introduced in Stage 6; "before" numbers will be produced by running the new sets against the v1 prompt* | — | Deterministic assertions |
