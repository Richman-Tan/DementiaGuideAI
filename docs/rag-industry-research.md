...
**Verdict for this system.** The architecture is already hybrid (correct choice — caregiver colloquialisms need semantic match; service names like "NASC", "iSupport" need lexical match). The production `match_chunks` body was captured 2026-07-17 (`scripts/migrations/2026-07-16_production_snapshot.sql`): it is a **weighted-sum** hybrid — `0.7 * (1 − cosine_distance) + 0.3 * ts_rank_cd(search_vector, websearch_to_tsquery(query))` — not RRF, with a tag-based pre-filter stage. This is the "workable but requires per-corpus weight tuning" family; the fixed 0.7/0.3 split is untuned and the two terms are on different scales (cosine ~0.3–0.5 here; `ts_rank_cd` unbounded), so the 0.3 keyword term can dominate on lexical matches. **Recommended experiment (Stage 10, not yet adopted):** benchmark an RRF variant against this weighted-sum on the deterministic retrieval metrics — RRF removes the scale-mismatch and the arbitrary weights. Adopt only on a measured win; both run in the existing Postgres function, no new infrastructure. The canonical committed version (Migration B) preserves the production scoring exactly and only moves the filter source from tags to provenance columns.
