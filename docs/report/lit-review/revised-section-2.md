...
2. **RQ7 (usability) stays** — the study is running, and recruitment was confirmed
   ongoing on 2026-09-21. It is written so the claim scales to whatever sample size
   is reached by the cutoff rather than promising a number. The current state of the
   database is recorded in [`../../study/data-state.md`](../../study/data-state.md).
...
**RQ7.** How do caregivers rate the usability and usefulness of the assistant, and
does the avatar condition differ from a text-only condition? *(Data collection is in
progress; reported at the sample size obtained by the cutoff in §2.4.)*
...
**Study scope and sample.** The usability study (RQ7, Objective 8) is designed,
ethically approved, piloted and **in progress**. Because recruitment for a study of
this kind runs to the end of the project, the report states the sample size reached
at a stated cutoff date and scales its claims to it: with a small sample the study is
reported descriptively, per participant, as a usability investigation rather than a
powered comparison, and the protocol and instruments are presented as a contribution
in their own right. Sessions run to debug the protocol are excluded from all
reported results, as is any session whose turn data was generated by an automated
test harness rather than by a participant.
...
| RQ7 | E7 *(in progress)* | Protocol and UAHPEC pack in `docs/study/`. Database as at 2026-09-21 (`docs/study/data-state.md`): 7 sessions, of which **three ran the full protocol** — both arms, six tasks, SUS for both arms, Likert and debrief. P06's 32 turns dated 18 Sep are the automated E4 batch and are excluded from turn-level measures. Six of seven sessions still carry `is_pilot = true`; that provenance must be resolved before any result is reported |
