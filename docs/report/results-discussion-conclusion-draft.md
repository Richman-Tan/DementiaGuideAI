...
- *RAG corpus imbalance (mitigated, not resolved).* The 387 bulk-loaded iSupport chunks initially displaced three hand-authored targets (retrieval 29 of 32). A source-family diversity cap recovered two of the three (31 of 32) and is now applied, but the underlying imbalance remains and the durable fix is to re-categorise the iSupport corpus. Groundedness was rated by an LLM judge (all 32 in-scope answers fully grounded), which is indicative but not definitive given same-family judge leniency; a human check is still advisable.
...
4. *Complete and extend the RAG evaluation.* Retrieval hit rate, boundary/out-of-scope handling, and LLM-judge groundedness are done (§6.9), and a source-family retrieval cap has been applied that raised retrieval to 31/32 (`rag_retrieval_rebalance_plan.md`). The remaining steps are a human check of the groundedness ratings (the LLM judge scored leniently, giving uniform full marks) and re-categorising the 387 iSupport caregiving chunks so the imbalance is fixed at the data layer rather than only by re-ranking.
