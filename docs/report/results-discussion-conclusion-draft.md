...
**Figure 2.** Distinct viseme shapes produced by the final pipeline (Unity Editor capture of the AaronImproved character): a bilabial closure with sealed lips, an open vowel driven by the merged open-mouth morph (with the facial hair following the jaw), and a labiodental lip-to-teeth contact, each annotated with the measured blendshape weight. A like-for-like baseline screenshot is not available because the character and scene camera were revised in the same change set; the before-and-after comparison is carried quantitatively by Figure 1 and Table 2.
...
- Figures generated by `scripts/make-figures.py` (reads the two TestResults runs above) into `docs/report/figures/` — `fig1_checks_passed.png`, `fig2_viseme_montage.png`, `fig3_bilabial_curves.png`; Fig 2 viseme face captures in `figures/_captures/`
- Latency instrumentation (stages / `[LATENCY SUMMARY]`): `src/features/voice/hooks/useAvatarConversation.js`
- RAG parameters (`TOP_K`, `MIN_SIMILARITY`, source-family cap, models): `src/lib/openaiService.js`; local knowledge base seed (70 chunks, 7 categories): `src/features/library/data/knowledgeBase.js`; deployed knowledge base (449 chunks incl. iSupport): Supabase `knowledge_chunks`
