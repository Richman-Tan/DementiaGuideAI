...
| `tts/` — `normalizeSpokenText`, `elevenLabsStreamService`                               | Text normalisation is pure; the stream service speaks the ElevenLabs WebSocket protocol using only `WebSocket`, which both runtimes provide. Audio _playback_ stays platform-side. |
...
per _folder_, which left partly-shared folders behind; it is now drawn per _file_.
