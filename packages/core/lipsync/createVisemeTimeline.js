...
import { PHONE_TO_VISEME, PHONE_WEIGHT_OVERRIDE, phoneDurationWeight } from './g2p/arpabetToViseme';
...
const DIGRAPH_VISEME = {
  ...
  oo: { viseme: 'ou', weight: 0.7 }, // "food", "book"
  ...
  ie: { viseme: 'ee', weight: 0.6 }, // "piece", "brief"
...
export function createVisemeTimeline(alignment, options = {}) {
  ...
  while (i < characters.length) {
        ...
        if (entry) {
          ...
          if (entry.viseme === 'neutral') {
            ...
            rawFrames.push({
              ...
              viseme: entry.viseme,
              duration,
              weight: entry.weight * digraphScale * wordScales[i],
            });
          }
          i += 2;
          continue;
    ...
    if (viseme === 'neutral') {
      ...
      const baseWeight =
        CHAR_WEIGHT_OVERRIDE[char] !== undefined
      ...
      rawFrames.push({
        ...
        duration,
        weight: DEFAULT_CONSONANT_WEIGHT * digraphScale * wordScales[i],
      });
    }
    ...
    if (ch && /[a-zA-Z']/.test(ch)) {
      ...
      if (phones && span > 0.005) {
        ...
        result.set(wordStart, {
          nextIndex: i,
          frames: phonesToFrames(
            phones,
            startTimes[wordStart],
            span,
            scale,
            resolvedWeights,
            digraphScale
...
function phonesToFrames(phones, wordStartTime, span, wordScale, resolvedWeights, digraphScale) {
  ...
  for (let k = 0; k < phones.length; k++) {
    ...
    if (!viseme || viseme === 'neutral') {
      ...
      const baseWeight =
        PHONE_WEIGHT_OVERRIDE[phone] !== undefined
...
function applyFinalLowering(frames, totalDuration) {
  ...
  const window = Math.min(totalDuration * 0.15, 0.25);
