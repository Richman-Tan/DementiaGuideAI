...
async function rpc(embedding, text, minSimilarity) {
  ...
  if (!r.ok)
    throw new Error(`match_chunks failed (${r.status}): ${(await r.text()).slice(0, 200)}`);
  return r.json();
}
...
async function main() {
  requireEnv();
  console.log(
    `Sweep: thresholds ${THRESHOLDS.join('/')} × caps ${CAPS.join('/')} over ${labelled.length} labelled questions`
  );
  console.log(
    `(production config: min_similarity=${MIN_SIMILARITY}, cap=${MAX_PER_SOURCE_FAMILY})\n`
  );
  ...
  for (const threshold of THRESHOLDS) {
    ...
    for (const cap of CAPS) {
      ...
      console.log(
        `min_sim=${threshold.toFixed(2)} cap=${cap}  ` +
          metricKeys.map((k) => `${k}=${agg[k]?.toFixed(4)}`).join('  ')
      );
    }
  }
  ...
  writeFileSync(
    outPath,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), gitSha: gitSha(), n: labelled.length, grid },
      null,
      2
    )
  );
