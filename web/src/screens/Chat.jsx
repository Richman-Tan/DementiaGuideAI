...
const Dots = () => (
  <div style={{ alignSelf: 'flex-start', background: 'var(--surface)', border: 'var(--bw) solid var(--border)', borderRadius: '18px 18px 18px 4px', padding: '16px 20px', display: 'flex', gap: '6px', boxShadow: 'var(--shadow)' }}>
    {[0, 0.15, 0.3].map((d) => (
      <span key={d} style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--primary)', animation: `dgBounce 1.2s ${d}s infinite` }} />
    ))}
  </div>
);
    ...
    <section style={{ display: 'flex', gap: '20px', height: isMobile ? 'calc(100vh - 100px)' : 'calc(100vh - 16px)', padding: '14px 0', boxSizing: 'border-box' }}>
      <div style={{ flex: '1', minWidth: '0', maxWidth: '760px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        <div ref={chatEl} style={{ flex: '1', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '16px', padding: '6px 4px 16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '14px', padding: '36px 16px', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', width: '100%', maxWidth: '420px' }}>
                {S.QUICK_QUESTIONS.slice(0, 3).map((q) => (
                  <button key={q} onClick={() => send(q)} style={{ minHeight: '48px', padding: '10px 16px', borderRadius: '14px', border: 'var(--bw) solid var(--border)', background: 'var(--surface)', color: 'var(--text)', cursor: 'pointer', textAlign: 'left' }} className="hv8">{q}</button>
                ))}
              </div>
            </div>
          )}
          {messages.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: 'var(--text2)', fontSize: '.82rem' }}><span style={{ flex: '1', height: '1px', background: 'var(--border)' }} />Today<span style={{ flex: '1', height: '1px', background: 'var(--border)' }} /></div>
          )}
          {messages.map((m, i) => {
                ...
                {isUser ? (
                  ...
                  <>
                    ...
                    {chips.length > 0 && !m.streaming && (
                      <div style={{ alignSelf: 'flex-start', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '8px', maxWidth: '86%' }}>
                        <span style={{ color: 'var(--text2)', fontSize: '.8rem', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '.05em' }}>Sources</span>
                        {chips.map((c, j) => (
                          <button key={j} onClick={() => setDrawer(c)} style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', minHeight: '36px', padding: '4px 14px', borderRadius: '999px', border: 'var(--bw) solid var(--border)', background: 'var(--surface)', color: 'var(--text)', fontSize: '.85rem', cursor: 'pointer' }} className="hv8"><span style={{ width: '8px', height: '8px', borderRadius: '50%', background: c.dot }} />{c.title}</button>
                        ))}
                      </div>
                    )}
                    <div style={{ alignSelf: 'flex-start', color: 'var(--text2)', fontSize: '.75rem', paddingLeft: '4px' }}>{m.time}</div>
                  </>
                )}
              </div>
            );
          ...
          {typing && <Dots />}
          {chatError && (
            <div style={{ alignSelf: 'flex-start', maxWidth: '86%', background: 'var(--surface)', border: 'var(--bw) solid var(--border)', borderRadius: '16px', padding: '16px 18px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '14px' }}>
      ...
      {showPanel && (
        <aside style={{ width: '230px', flexShrink: '0', alignSelf: 'flex-start', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px', background: 'var(--surface)', border: 'var(--bw) solid var(--border)', borderRadius: '20px', padding: '22px', boxShadow: 'var(--shadow)', textAlign: 'center' }}>
          {settings.showAvatar && (
